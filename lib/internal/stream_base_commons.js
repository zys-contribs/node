...
function handleWriteReq(req, data, encoding) {
  ...
  switch (encoding.length) {
    case 4:
      if (encoding === 'utf8') return handle.writeUtf8String(req, data);
      if (encoding === 'ucs2') return handle.writeUcs2String(req, data);
      break;
    case 5:
      if (encoding === 'utf-8') return handle.writeUtf8String(req, data);
      if (encoding === 'ascii') return handle.writeAsciiString(req, data);
      if (encoding === 'ucs-2') return handle.writeUcs2String(req, data);
      break;
    case 7:
      if (encoding === 'utf16le') return handle.writeUcs2String(req, data);
      break;
    case 8:
      if (encoding === 'utf-16le') return handle.writeUcs2String(req, data);
      break;
    case 6:
      if (encoding === 'buffer') {
